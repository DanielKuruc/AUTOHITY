...
import { ActivityIndicator, Image, Linking, ProgressBarAndroid, StyleSheet, Text, TouchableOpacity, View } from 'react-native';
...
export function PurchaseCard({ purchase, isPending, progress = 0, status, onRetry }: PurchaseCardProps) {
  ...
  const formatDate = (dateString?: string | null) => {
    ...
    const czechMatch = dateString.match(/^(\d{1,2})\.(\d{1,2})\.(\d{4})(?:\s+(\d{1,2}):(\d{2}))?/);
    if (czechMatch) {
  ...
  if (isTablet) {
    return (
      <TouchableOpacity 
        style={[styles.tabletCard, { backgroundColor: theme.card, opacity: cardOpacity, flex: 1 }]} 
        onPress={handlePress} 
        activeOpacity={0.7}
        disabled={isPending}
      >
        {/* Top - Large Image */}
        <View style={[styles.tabletImageContainer, { backgroundColor: theme.inputBackground }]}>
        ...
        {/* Bottom - Info section */}
        <View style={styles.tabletInfoSection}>
          {/* Title row - Make/Model + State */}
          <View style={styles.tabletTitleRow}>
            <View style={{ flex: 1 }}>
              {purchase.carDetails && (
                <Text style={[styles.tabletCarName, { color: theme.text }]} numberOfLines={1}>
                  {purchase.carDetails.make} {purchase.carDetails.model}
                </Text>
              )}
              <Text style={[styles.tabletClientName, { color: '#E30613' }]} numberOfLines={1}>
                {purchase.clientName}
              </Text>
            </View>
            <View style={[styles.tabletStateBadge, { backgroundColor: stateColor + '20' }]}>
              <Text style={[styles.tabletStateText, { color: stateColor }]}>
          ...
          <View style={styles.tabletPriceContainer}>
            {purchase.purchaseState === PurchaseState.COMPLETED && purchase.totalAmount && (
              <Text style={[styles.tabletPrice, { color: '#FFFFFF' }]}>
          ...
          <View style={styles.tabletDetailsRow}>
            <View style={styles.detailItem}>
              <Text style={[styles.detailLabel, { color: theme.textTertiary, fontSize: 9, fontWeight: '700', textTransform: 'uppercase' }]}>Rok</Text>
              <Text style={[styles.detailValue, { color: theme.text }]}>
                {(purchase.carDetails as any)?.firstRegistration 
                  ? new Date((purchase.carDetails as any).firstRegistration).getFullYear() 
                  : purchase.carDetails?.year || 'N/A'}
              </Text>
            </View>
            <View style={styles.detailItem}>
              <Text style={[styles.detailLabel, { color: theme.textTertiary, fontSize: 9, fontWeight: '700', textTransform: 'uppercase' }]}>Km</Text>
              <Text style={[styles.detailValue, { color: theme.text }]}>
                {purchase.carDetails?.mileage ? purchase.carDetails.mileage.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ' ') : 'N/A'}
              </Text>
            </View>
            <View style={styles.detailItem}>
              <Text style={[styles.detailLabel, { color: theme.textTertiary, fontSize: 9, fontWeight: '700', textTransform: 'uppercase' }]}>Palivo</Text>
              <Text style={[styles.detailValue, { color: theme.text, fontSize: 10 }]}>
                {purchase.carDetails?.fuelType || 'N/A'}
              </Text>
            </View>
            <View style={styles.detailItem}>
              <Text style={[styles.detailLabel, { color: theme.textTertiary, fontSize: 9, fontWeight: '700', textTransform: 'uppercase' }]}>Převod</Text>
              <Text style={[styles.detailValue, { color: theme.text }]}>
          ...
          {/* Badges row */}
          {(purchase.carDetails?.hasServiceBook || purchase.carDetails?.isFirstOwner || purchase.carDetails?.isImport === false || purchase.isVatPayer) && (
            <View style={styles.tabletBadgesRow}>
              {purchase.carDetails?.hasServiceBook && (
                <View style={styles.tabletBadge}>
                  <Ionicons name="checkmark-circle" size={14} color={theme.success} />
                  <Text style={[styles.tabletBadgeText, { color: theme.success }]}>Servisní kniha</Text>
                </View>
              )}
              {purchase.carDetails?.isFirstOwner && (
                <View style={styles.tabletBadge}>
                  <Ionicons name="checkmark-circle" size={14} color={theme.success} />
                  <Text style={[styles.tabletBadgeText, { color: theme.success }]}>1. majitel</Text>
                </View>
              )}
              {purchase.carDetails?.isImport === false && (
                <View style={styles.tabletBadge}>
                  <Ionicons name="checkmark-circle" size={14} color={theme.success} />
                  <Text style={[styles.tabletBadgeText, { color: theme.success }]}>ČR</Text>
                </View>
              )}
              {purchase.isVatPayer && (
                <View style={styles.tabletBadge}>
                  <Ionicons name="checkmark-circle" size={14} color={theme.success} />
                  <Text style={[styles.tabletBadgeText, { color: theme.success }]}>DPH</Text>
                </View>
              )}
            </View>
          )}
            ...
            <View style={styles.tabletEmployeeRow}>
              {employee && (
                <View style={styles.employeeInfoRow}>
                  ...
                  <Text style={[styles.tabletEmployeeText, { color: theme.textSecondary }]}>
                    {employee.lastName}
                  </Text>
                </View>
              )}
              {displayDate && (
                <Text style={[styles.tabletEmployeeText, { color: theme.textSecondary }]}>
                  {formatDate(displayDate).split(' ')[0]} {displayTime}
                </Text>
              )}
            </View>
          )}
          ...
          <View style={styles.tabletButtonsRow}>
            {purchase.purchaseState === PurchaseState.IN_PROGRESS && (
              <TouchableOpacity
                style={[styles.tabletActionBtn, { backgroundColor: theme.accent }]}
                onPress={() => router.push(`/purchase/edit-purchase?id=${purchase.id}`)}
              >
                <Ionicons name="pencil" size={16} color="#FFFFFF" />
                <Text style={styles.tabletActionBtnText}>Upravit</Text>
              </TouchableOpacity>
            )}
            {(purchase.purchaseState === PurchaseState.NEW ||
              ...
              <TouchableOpacity
                style={[styles.tabletActionBtn, { backgroundColor: theme.success }]}
                onPress={() => {
                  if (purchase.phone) {
              ...
              >
                ...
                <Text style={styles.tabletActionBtnText}>Zavolat</Text>
              </TouchableOpacity>
            )}
          </View>
        </View>
        ...
        {isPending && (
          <View style={[StyleSheet.absoluteFillObject, styles.pendingOverlay, { backgroundColor: theme.card + 'cc', borderRadius: 12, justifyContent: 'center', alignItems: 'center' }]}>
            <ActivityIndicator size="large" color={theme.accent} />
            <Text style={{ color: theme.text, marginTop: 8 }}>Probíhá nahrávání...</Text>
          </View>
        )}
      </TouchableOpacity>
    );
  ...
  return (
    <TouchableOpacity 
      style={[styles.card, { backgroundColor: theme.card, opacity: cardOpacity }]} 
      onPress={handlePress} 
      activeOpacity={0.7}
      disabled={isPending}
    >
        ...
        <View style={styles.infoContainer}>
          {purchase.carDetails && (
            <Text style={[styles.carName, { color: theme.text }]} numberOfLines={1}>
              {purchase.carDetails.make} {purchase.carDetails.model}
              {(purchase.carDetails as any).firstRegistration && ` (${new Date((purchase.carDetails as any).firstRegistration).getFullYear()})`}
            </Text>
          )}
          ...
          <View style={styles.dateTimeRow}>
            ...
            <Text style={[styles.time, { color: theme.textTertiary }]}> | </Text>
            <Text style={[styles.time, { color: theme.textTertiary }]}>
              {displayTime}
            </Text>
          </View>
          {/* Cancellation reason */}
          {purchase.purchaseState === PurchaseState.CANCELLED && purchase.notes && (
            <View style={styles.cancellationReasonRow}>
              <Ionicons name="alert-circle" size={12} color={theme.error} />
              <Text style={[styles.cancellationReasonText, { color: theme.error }]} numberOfLines={1}>
                {purchase.notes.split('Důvod zrušení:')[1]?.trim() || 'Bez uvedeného důvodu'}
              </Text>
            </View>
          )}
          {/* Employee surname */}
          {employee && (
            <View style={styles.employeeRow}>
              <Ionicons name="person" size={12} color={theme.textSecondary} />
              <Text style={[styles.employeeText, { color: theme.textSecondary }]} numberOfLines={1}>
                {employee.lastName}
              </Text>
            </View>
          )}
            ...
            <View style={styles.uploadStatusContainer}>
              {status === 'uploading' && (
                <>
                  ...
                  />
                  <Text style={[styles.uploadText, { color: theme.textTertiary }]}>
                    Nahrávání... {Math.round(progress * 100)}%
                  </Text>
                </>
              )}
              {status === 'error' && (
                <TouchableOpacity style={styles.retryButton} onPress={() => onRetry?.(purchase.id)}>
              ...
              {status === 'success' && (
                <View style={styles.successContainer}>
                  ...
                  <Text style={[styles.uploadText, { color: theme.success }]}>Nahráno</Text>
                </View>
              )}
            </View>
          )}
        ...
        <View style={styles.rightContainer}>
          ...
          <View style={styles.buttonsRow}>
            ...
            {(purchase.purchaseState === PurchaseState.NEW ||
              ...
              purchase.purchaseState === PurchaseState.CANCELLED) && (
              <TouchableOpacity
                style={[styles.phoneButton, { backgroundColor: theme.success }]}
                onPress={() => {
                  if (purchase.phone) {
                    Linking.openURL(`tel:${purchase.phone}`);
                  }
