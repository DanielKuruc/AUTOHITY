...
export default function PurchaseDetailScreen() {
  ...
  const { isSplitView, isCompact, isWeb } = useTabletLayout();
  const purchase = getPurchaseById(id);
  const [selectedImageIndex, setSelectedImageIndex] = useState<number | null>(
    null,
  );
  const [selectedDefectImageIndex, setSelectedDefectImageIndex] = useState<
    number | null
  >(null);
  const [serviceNotesTemp, setServiceNotesTemp] = useState(
    (purchase as any)?.serviceNotes || "",
  );
  const [generalNotesTemp, setGeneralNotesTemp] = useState(
    purchase?.notes || "",
  );

  useEffect(() => {
    setServiceNotesTemp((purchase as any)?.serviceNotes || "");
    setGeneralNotesTemp(purchase?.notes || "");
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [purchase?.serviceNotes, purchase?.notes]);
  ...
  const [completeExpectedSalePrice, setCompleteExpectedSalePrice] =
    useState("");
  // Cancel modal state
  ...
  const getNextState = (currentState: PurchaseState): PurchaseState | null => {
    switch (currentState) {
      case PurchaseState.NEW:
        return PurchaseState.IN_PROGRESS;
      case PurchaseState.IN_PROGRESS:
  ...
  const handleStateUpdate = () => {
    ...
    if (nextState === PurchaseState.COMPLETED) {
      ...
      setCompleteExpectedSalePrice(
        purchase.expectedSalePrice?.toString() || "",
      );
      setShowCompleteModal(true);
    } else {
      // Pro ostatní stavy standardní alert
      showAlert("Aktualizovat stav", "Zahájit zpracování tohoto výkupu?", [
  ...
  const handleCompletePurchase = async () => {
    ...
    const expectedSaleValue = completeExpectedSalePrice
      ? parseInt(completeExpectedSalePrice.replace(/\s/g, ""))
      : undefined;
    ...
    await updatePurchase(purchase.id, {
      ...
      isPriority: 0, // Vypnout prioritu při dokončení
    });
  ...
  const handleConfirmCancelPurchase = async () => {
    ...
    const reasonText =
      cancelReason + (cancelReasonNote ? ` - ${cancelReasonNote}` : "");
    await updatePurchase(purchase.id, {
  ...
  const car = purchase.carDetails || {
    ...
    year: (purchase as any).vehicleYear
      ? Number((purchase as any).vehicleYear)
      : undefined,
    mileage: (purchase as any).vehicleMileage
      ? Number((purchase as any).vehicleMileage)
      : undefined,
    hasServiceBook: false,
    isFirstOwner: false,
    isImport: false,
    cebia: (purchase as any).cebia || false,
    caVertical: (purchase as any).caVertical || false,
    registered: (purchase as any).registered ?? true,
  };
  const getCancelReasonText = (): string | null => {
    if (purchase.purchaseState !== PurchaseState.CANCELLED || !purchase.notes)
      return null;
  ...
  const renderInfoRow = (
    ...
    const showWarningIcon =
      (label === "Počet vlastníků" || label === "Počet provozovatelů") &&
      Number(value) >= 3;
      ...
      <View style={[styles.infoRow, { borderBottomColor: theme.borderLight }]}>
        {icon && (
          <View
            style={[
              styles.infoIcon,
              { backgroundColor: theme.idIconsBackground },
            ]}
          >
            <Ionicons name={icon as any} size={16} color={theme.idIcons} />
          </View>
        )}
        ...
        {(label === "Telefon" || label === "Výkupčí") && onCallPress && (
          <TouchableOpacity
            accessibilityLabel={
              label === "Telefon" ? "Zavolat klientovi" : "Zavolat výkupčímu"
            }
            onPress={onCallPress}
            style={[
              styles.callButton,
              { backgroundColor: theme.idButtonBackground },
            ]}
          >
  ...
  // ==== Kartičky, které se na WEBU přesouvají do pravého sloupce ====
  const zakladniInformaceCard = (
    <View style={[styles.section, isCompact && styles.sectionCard, { backgroundColor: theme.card }]}>
      ...
      {purchase?.employeeId &&
        ...
        (() => {
          const employeeInfo = users.find(
            (u) => u.id === purchase.employeeId,
          );
          return renderInfoRow(
            ...
            employeeInfo?.phoneNumber
              ? () => {
                  Linking.openURL(`tel:${employeeInfo.phoneNumber}`);
                }
              : undefined,
          );
        })()}
      {purchase && renderInfoRow(
        "Datum výkupu",
      ...
      {purchase && renderInfoRow(
        "Datum prohlídky",
        formatDate(purchase.inspectionDate),
        "time-outline",
      )}
      {purchase && renderInfoRow(
        "Cena zákazník",
      ...
      {purchase && renderInfoRow(
        "Cena nabídnuta",
        formatAmount(purchase.offeredPrice),
        "cash-outline",
      )}
      {purchase && renderInfoRow(
        "Před. cena prodeje",
        formatAmount(purchase.expectedSalePrice),
        "trending-up-outline",
      )}
      {purchase && renderBooleanRow(
        "Protiúčet",
        purchase.isCounterAccount,
        "swap-horizontal-outline",
      )}
      {renderBooleanRow(
        "Registrováno",
        (car as any)?.registered ?? true,
        "checkbox-outline",
      )}
      {((car as any)?.cebia !== undefined ||
        (car as any)?.caVertical !== undefined) && (
        <>
          <View
            style={[
              styles.infoRow,
      ...
      {purchase && renderInfoRow(
        "Odkud zná",
        purchase.sourceKnowledge,
        "megaphone-outline",
      )}
    </View>
  );

  const klientCard = purchase ? (
    <View style={[styles.section, isCompact && styles.sectionCard, { backgroundColor: theme.card }]}>
      <View style={styles.sectionHeader}>
        <View
          style={[
        ...
        >
          <Ionicons name="person" size={18} color={theme.idIcons} />
        </View>
        <Text style={[styles.sectionTitle, { color: theme.text }]}>
      ...
      {renderInfoRow(
        "Typ",
        purchase.clientType === "company" ? "Firma" : "Fyzická osoba",
        "business-outline",
      )}
      {renderBooleanRow(
        "Plátce DPH",
  ...
  const actionsCard = purchase ? (
    <View style={styles.actionsSection}>
      {purchase.purchaseState === PurchaseState.IN_PROGRESS && (
        <TouchableOpacity
          style={[styles.editButton, { backgroundColor: theme.warning }]}
          onPress={() =>
  ...
  return (
    ...
    >
          ...
          {purchase.purchaseState === PurchaseState.IN_PROGRESS && (
            <TouchableOpacity
              style={[
                ...
                {
                  borderColor: theme.headerButtonBackground,
                  backgroundColor: theme.headerButtonBackground,
                },
              ]}
              onPress={() =>
      ...
      <View style={[isSplitView && !isWeb ? styles.splitDetailLayout : { flex: 1 }]}>
        {/* Sidebar na iPadu - summary info (na webu nahradí pravý sloupec) */}
        {isSplitView && !isWeb && (
          <ScrollView
            style={[
            ...
            contentContainerStyle={styles.detailSidebarContent}
          ...
          >
            <View style={styles.sidebarSummary}>
              ...
              >
                ...
                {(car as any)?.engineSize && `${(car as any).engineSize} • `}
                {car.year && `${car.year}`}
              </Text>
              <Text
                style={[
                  styles.sidebarMuted,
                  { color: theme.text, fontWeight: "600", marginTop: 4 },
                ]}
              >
            ...
            <View
              style={[styles.sidebarDivider, { backgroundColor: theme.border }]}
            />
            <View style={styles.sidebarSummary}>
              <Text
                style={[styles.sidebarLabel, { color: theme.textSecondary }]}
              >
                KLIENT
              </Text>
            ...
            {(car?.hasServiceBook ||
              ...
              <>
                <View
                  style={[
                    styles.sidebarDivider,
                    { backgroundColor: theme.border, marginTop: 12 },
                  ]}
                />
                <View style={styles.sidebarStatusIcons}>
                  ...
                  {car?.isImport === false && (
                    <View style={styles.sidebarStatusIcon}>
                      <Text
                        style={[styles.sidebarStatusEmoji, { fontSize: 20 }]}
                      >
                        🇨🇿
                      </Text>
                      <Text
            ...
            <View style={styles.sidebarActions}>
              ...
              {users.find((u) => u.id === currentUser?.id)?.isAdmin && (
                ...
                >
                  ...
                  <Text
                    style={[
                      styles.sidebarActionButtonText,
                      { color: "#FFFFFF" },
                    ]}
                  >
                    Odstranit
        ...
        <ScrollView
          ...
          contentContainerStyle={[
            ...
            isCompact && styles.scrollContentWeb,
          ]}
        >
          <View style={(isCompact && isWeb) ? styles.webTwoColumnOuter : undefined}>
          <View style={(isCompact && isWeb) ? styles.webMainColumn : undefined}>
          ...
          {((purchase as any).coverPhotoUri ||
            ...
            <View style={styles.heroGallery}>
              <TouchableOpacity
                style={[
                  !isCompact && styles.mainImageContainer,
                  isCompact && styles.mainImageContainerWeb,
                ]}
                onPress={() => {
              ...
              >
                <ExpoImage
                  source={{
                    uri:
                      (purchase as any).coverPhotoUri || purchase.images?.[0],
                  }}
                  style={[styles.mainImage, isCompact && styles.mainImageWeb]}
                ...
                <View style={styles.imageOverlay}>
                  ...
                  <View style={styles.photoCount}>
                    ...
                    <Text style={styles.photoCountText}>
                      {((purchase as any).coverPhotoUri ? 1 : 0) +
              ...
              {(((purchase as any).coverPhotoUri &&
                ...
                >
                  ...
                  {purchase.images &&
                    purchase.images.map((imageUrl, index) => (
                      <TouchableOpacity
                        key={`vehicle-${index}`}
                        style={styles.thumbSmall}
          ...
          {/* Hero Card */}
          <View style={[styles.heroCard, isCompact && styles.heroCardWeb, { backgroundColor: theme.card }]}>
            {!(purchase.images && purchase.images.length > 0) && (
              <View
                style={[
                  styles.statusBadge,
                  {
                    backgroundColor: getStateColor(purchase.purchaseState),
            ...
            {(car as any) &&
              (car.make || car.model || (car as any).firstRegistration) && (
                <Text style={[styles.vehicleNameLarge, { color: theme.text }]}>
                  {car.make} {car.model}
                  {(car as any).fuelType && ` ${(car as any).fuelType}`}
                  {(car as any).firstRegistration &&
                    ` (${new Date((car as any).firstRegistration).getFullYear()})`}
                </Text>
              )}

            <Text style={[styles.spzSmall, { color: theme.textTertiary }]}>
              {purchase.carDetails?.spz ? "🇨🇿 " : ""}
              {formatSpz(purchase.carDetails?.spz)}
            </Text>
            ...
            <View style={styles.heroStats}>
              <View style={styles.heroStat}>
                ...
                <Text
                  style={[styles.heroStatLabel, { color: theme.textSecondary }]}
                >
                  {purchase.purchaseState === PurchaseState.COMPLETED
                    ? "Výkupní"
                    : "Nabídnuta"}
            ...
            <View style={styles.heroStatusIcons}>
              ...
              <View style={styles.statusIconItem}>
                <Text style={[styles.statusIconEmoji, { fontSize: 20 }]}>
                  🇨🇿
                </Text>
                <Text
                  style={[
          ...
          <View style={isCompact ? [styles.sectionsGrid, isWeb && (styles.sectionsGridWeb as any)] : undefined}>

          {/* Priority Toggle - HNE POD HERO KARTOU - ONLY for ADMINS */}
          {((currentUser &&
            users.find((u) => u.id === currentUser.id)?.isAdmin) ||
            purchase.isPriority === 1) && (
            <View style={[styles.section, isCompact && styles.sectionCard, { backgroundColor: theme.card }]}>
              ...
              >
                ...
                users.find((u) => u.id === currentUser.id)?.isAdmin ? (
                  <Switch
                    value={purchase.isPriority === 1}
                    onValueChange={async (value) => {
                      await updatePurchase(purchase.id, {
                        isPriority: value ? 1 : 0,
                      });
                      if (value) {
                        showToast("Vozidlo označeno jako prioritní", "success");
                        try {
                          const vehicleInfo = `${purchase.carDetails?.make || "?"} ${purchase.carDetails?.model || "?"} (${purchase.carDetails?.spz})`;
                          await apiService.post("/send_push.php", {
                            title: "🚨 Nové prioritní vozidlo",
                            body: `${vehicleInfo} - ${purchase.clientName}`,
                            data: {
                          ...
                          // Uložit notifikaci do DB (globální - user_id = 0)
                          await fetch(`${API_BASE_URL}/notifications.php`, {
                            ...
                            body: JSON.stringify({
                              ...
                              title: "🚨 Nové prioritní vozidlo",
                              body: vehicleInfo,
                              notification_type: "priority_purchase",
                              data: {
                                purchaseId: purchase.id,
                          ...
                          // Error sending notification - silent fail
                        }
                      } else {
                        showToast("Priorita byla odstraněna", "info");
                      }
                    }}
          ...
          {/* Základní informace - na webu se renderuje v pravém sloupci */}
          {!(isCompact && isWeb) && zakladniInformaceCard}
          {/* Informace o vozidle */}
          {car &&
            (car.make ||
              car.model ||
              ...
              <View style={[styles.section, isCompact && styles.sectionCard, { backgroundColor: theme.card }]}>
                ...
                {renderInfoRow(
                  ...
                  (car as any).vin
                    ? () => {
                        Clipboard.setStringAsync((car as any).vin)
                          .then(() => {
                ...
                {renderInfoRow("SPZ", (car as any).spz, "document-outline")}
                {renderInfoRow(
                  "Palivo",
                  (car as any).fuelType,
                  "flame-outline",
                ...
                {renderInfoRow(
                  "První registrace",
                  formatDate((car as any).firstRegistration),
                  "time-outline",
                )}
                {renderBooleanRow(
                  "Dovoz",
                ...
                {renderInfoRow(
                  "Počet vlastníků",
                  (car as any).pocetVlastniku,
                  "people-outline",
                )}
                {renderInfoRow(
                  "Počet provozovatelů",
                  (car as any).pocetProvozovatelu,
                  "people-outline",
                )}
              </View>
            )}

          {/* Obecná poznámka - vždy editovatelná */}
          <View style={[styles.section, isCompact && styles.sectionCard, { backgroundColor: theme.card }]}>
            <View style={styles.sectionHeader}>
              <View
                style={[
            ...
            >
              ...
              >
                <TextInput
                  ...
                  value={generalNotesTemp}
                  onChangeText={(newNotes) => {
                    setGeneralNotesTemp(newNotes);
                  }}
                  placeholder="Zadejte obecnou poznámku..."
              ...
              <TouchableOpacity
                style={[
                  styles.notesEditSaveButton,
                  { backgroundColor: theme.success },
                ]}
                onPress={async () => {
                  try {
                    await updatePurchase(purchase.id, { notes: generalNotesTemp });
                    showToast("Poznámka uložena", "success");
                  } catch (error) {
                    showToast("Chyba při ukládání poznámky", "error");
                    console.log("Error saving general notes:", error);
                  }
                }}
              >
          ...
          <View style={[styles.section, isCompact && styles.sectionCard, { backgroundColor: theme.card }]}>
            ...
            {Array.isArray((purchase as any).componentStatuses) &&
              (purchase as any).componentStatuses.length > 0 &&
              (() => {
                ...
                return filteredComponents.map((item: any, idx: number) => {
                  ...
                  const conf =
                    STATUS_OPTIONS[statusKey] || STATUS_OPTIONS["good"];
                  const displayName =
                    componentNameMap[item.component] || item.component;
                  return (
                    <View
            ...
            <TouchableOpacity
              style={[styles.paintThicknessRow, { borderTopColor: theme.borderLight }]}
              onPress={() => setShowPaintThickness(true)}
            >
              <View
                style={[
                  styles.paintThicknessIcon,
                  { backgroundColor: theme.idIconsBackground },
                ]}
              >
                <Ionicons name="color-fill-outline" size={18} color={theme.idIcons} />
              </View>
              <Text style={[styles.paintThicknessLabel, { color: theme.text }]}>
                Tloušťky laku
              </Text>
              <Ionicons
            ...
            >
              ...
              <TouchableOpacity
                ...
                onPress={async () => {
                  try {
                    await updatePurchase(purchase.id, {
                      serviceNotes: serviceNotesTemp,
                    });
                    showToast("Servisní poznámka uložena", "success");
                  } catch (error) {
                    showToast("Chyba při ukládání servisní poznámky", "error");
                    console.log("Error saving service notes:", error);
                  }
                }}
              >
                <Ionicons name="checkmark" size={30} color="#FFFFFF" />
          ...
          {/* Klient - na webu se renderuje v pravém sloupci */}
          {!(isCompact && isWeb) && klientCard}
          ...
          {purchase.isCounterAccount && (purchase as any).counterAccountCar && (
            <View style={[styles.section, isCompact && styles.sectionCard, { backgroundColor: theme.card }]}>
              ...
              {renderInfoRow(
                "Značka",
                (purchase as any).counterAccountCar.make,
                "car-sport-outline",
              )}
              {renderInfoRow(
                "Model",
                (purchase as any).counterAccountCar.model,
                "car-outline",
              )}
              {renderInfoRow(
                "Motorizace",
                (purchase as any).counterAccountCar.variant,
                "speedometer-outline",
              )}
              {renderInfoRow(
                "Cena",
                (purchase as any).counterAccountCar.price
                  ? new Intl.NumberFormat("cs-CZ", {
                      style: "currency",
                      currency: "CZK",
                      maximumFractionDigits: 0,
                    }).format((purchase as any).counterAccountCar.price)
                  : undefined,
                "cash-outline",
              )}
            </View>
          ...
          {employee && (
            <View style={[styles.section, isCompact && styles.sectionCard, { backgroundColor: theme.card }]}>
              <View style={styles.sectionHeader}>
                ...
                >
                  <Ionicons name="people" size={18} color={theme.idIcons} />
                </View>
                <Text style={[styles.sectionTitle, { color: theme.text }]}>
                  Přiřazený pracovník
                </Text>
              ...
              <View style={styles.employeeCard}>
                ...
                {currentUser?.phoneNumber && (
                  <TouchableOpacity
                    style={[
                      styles.callEmployeeButton,
                      { backgroundColor: theme.accent },
                    ]}
                    onPress={() =>
                      Linking.openURL(`tel:${currentUser.phoneNumber}`)
                    }
                  >
                    <Ionicons name="call" size={16} color="#FFFFFF" />
                  </TouchableOpacity>
          ...
          {/* konec webMainColumn (pouze web) */}
          </View>
          {/* Pravý sloupec – jen na webu: Základní informace + Klient + Akce */}
          {(isCompact && isWeb) && (
            <View style={styles.webRightColumn}>
              {zakladniInformaceCard}
              {klientCard}
              {actionsCard}
            </View>
          )}
          {/* konec webTwoColumnOuter (pouze web) */}
          </View>

          {/* Akce - skryté na split-view (iPad má je v levém sidebaru) a na webu (má je v pravém sloupci) */}
          {!isSplitView && !(isCompact && isWeb) && actionsCard}
      ...
      {selectedImageIndex !== null &&
        (((purchase as any).coverPhotoUri && selectedImageIndex === -1) ||
          (purchase.images && selectedImageIndex >= 0)) && (
          <ImageGallery
            images={[
              ...((purchase as any).coverPhotoUri
                ? [(purchase as any).coverPhotoUri]
            ...
            initialIndex={
              selectedImageIndex === -1
                ? 0
                : selectedImageIndex + ((purchase as any).coverPhotoUri ? 1 : 0)
            }
          ...
          <View style={[styles.modalContent, { backgroundColor: theme.card }]}>
            ...
            <View style={styles.modalActions}>
              ...
              <TouchableOpacity
                ...
                onPress={async () => {
                  const reasonText =
                    cancelReason === "Jiné" && cancelReasonNote.trim()
                      ? cancelReasonNote.trim()
                      : cancelReason;
                  const notesPrefix = purchase.notes
                    ? purchase.notes + "\n"
                    : "";
                  await updatePurchase(purchase.id, {
                    purchaseState: PurchaseState.CANCELLED,
                    notes: `${notesPrefix}Důvod zrušení: ${reasonText}`,
                    isPriority: 0, // Vypnout prioritu při odmítnutí
                  });
                  setShowCancelModal(false);
      ...
      <PaintThicknessModal
        ...
        readings={purchase.paintThickness}
        onSave={async (readings) => {
          try {
            await updatePurchase(purchase.id, { paintThickness: readings });
            showToast("Tloušťky laku uloženy", "success");
          } catch (error) {
            showToast("Chyba při ukládání tloušťek laku", "error");
            console.log("Error saving paint thickness:", error);
          }
        }}
      />
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  mainImageWeb: { backgroundColor: "rgba(0,0,0,0.35)" },
  imageOverlay: {
    position: "absolute",
    top: 0,
    left: 0,
  ...
  sectionCard: {
    ...
    flexGrow: 1,
    minWidth: 320,
    ...(Platform.OS === "web" ? { breakInside: "avoid", marginBottom: 12 } as any : {}),
  },
  // WEB dvousloupcový layout (main + right column jako v referenci "Your Journey")
  webTwoColumnOuter: {
    flexDirection: "row",
    alignItems: "flex-start",
    gap: 16,
  ...
  paintThicknessLabel: { flex: 1, fontSize: 15, fontWeight: "500" },
